<template>
  <el-breadcrumb class="app-breadcrumb" separator="→" style="height:50px;backgroundColor:#ffffff;borderRadius:6px;padding:0px 20px 0px 20px;boxShadow:0px 0px 8px #d9efec;borderWidth:0;borderStyle:solid;borderColor:#30c7ba;">
    <transition-group name="breadcrumb" class="box" :style="1==1?'justifyContent:flex-start;':1==2?'justifyContent:center;':'justifyContent:flex-end;'">
      <el-breadcrumb-item v-for="(item,index) in levelList" :key="item.path">
        <span v-if="item.redirect==='noRedirect'||index==levelList.length-1" class="no-redirect" style="color:#30c7ba">{{ item.name }}</span>
